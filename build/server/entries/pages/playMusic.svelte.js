...
<div class="${escape(!(embedList.length === 0) ? "visible" : "invisible") + " overflow-x-auto h-auto w-full flex border"}">${each(embedList, (m) => {
      return `<div class="${"grow-0 shrink-0 h-auto w-48 bg-c-dark-purple text-white border relative cursor-pointer"}"><span class="${"absolute right-1 top-1 z-10"}">${validate_component(Close, "Close").$$render($$result, {}, {}, {})}</span>
